...
for (a, b, time) in links:
    ...
    if slice_id == 1+prev_slice_id and slice_id > 0:
        ...
        print ("# nodes in slice", slice_id, len(slices_links[snapshot_id-1].nodes()))
        print ("# edges in slice", slice_id, len(slices_links[snapshot_id-1].edges()))
...
def remap(slices_graphs, slices_features):  # map original node_id (4,5,10,12) to continue ids (0,1,2,3)
    snapshots = []
