...
def _graph_profile(N_snapshots):
    ...
    plt.xlabel('Number of Structural Neighbors',fontsize=17)
    plt.ylabel('CDF')
    ...
    CDF = np.cumsum(Y)
    plt.plot(X, CDF)
    plt.xlabel('Length of Vertex Sequence',fontsize=17)
    plt.ylabel('CDF', fontsize=17)
    plt.savefig(current_path + '/tem_cdf.eps', dpi=900,bbox_inches='tight')
    ...
    plt.xlabel('Number of Edges', fontsize=17)
    plt.ylabel('CDF', fontsize=17)
    plt.savefig(current_path + '/str_cdf.eps', dpi=900,bbox_inches='tight')
