...
class GAT_Layer_train(nn.Module):
    def __init__(self, 
                args,
                input_dim, 
                output_dim, 
                ...
                ffd_drop,
                residual):
        super(GAT_Layer_train, self).__init__()
        self.args = args
        self.out_dim = output_dim // n_heads
        self.n_heads = n_heads
        self.act = nn.ELU()
...
class GAT_Layer_test(nn.Module):
    def __init__(self, 
                args,
                input_dim, 
                output_dim, 
                ...
                ffd_drop,
                residual):
        super(GAT_Layer_test, self).__init__()
        self.args = args
        self.out_dim = output_dim // n_heads
        self.n_heads = n_heads
        self.act = nn.ELU()
...
class ATT_layer_train(nn.Module):
    '''
    inputs may have different time-series lengths
    Method 1: Pad zero to make all inputs have the same lengths
    '''
    def __init__(self,
                ...
                residual,
                interval_ratio):
        super(ATT_layer_train, self).__init__()
        self.n_heads = n_heads
        self.residual = residual
        # self.sample_masks = sample_masks
        self.method = method
        ...
        # define weights
        self.position_embeddings = nn.Parameter(torch.Tensor(1, input_dim))
        # self.time_weights = nn.Parameter(torch.Tensor(num_time_steps, num_time_steps))
        self.Q_embedding_weights = nn.Parameter(torch.Tensor(input_dim, input_dim))
        self.K_embedding_weights = nn.Parameter(torch.Tensor(input_dim, input_dim))
        self.V_embedding_weights = nn.Parameter(torch.Tensor(input_dim, input_dim))
    ...
    def forward(self, inputs):
        ...
        # P = torch.ones(1, 2).long()
        # # A = torch.arange(0, 10).reshape(1, -1).repeat(5, 1).reshape(5, 10, 1).long()
        # A = torch.arange(0, 10).reshape(-1, 1).long()
        # P_A = torch.mm(A, P).reshape(1, 10, 2)
        # PP = P_A.repeat(5, 1, 1)
        # # print(PP)

        # # print(A.size(), P.size())
        # # P_A = torch.mm(A, P)
        # B = torch.zeros(5, 10, 2)
        self.num_time_steps = inputs.shape[1]
        start = time.time()
        # position_inputs = torch.arange(0, inputs.shape[1]).reshape(-1, 1).long().to(inputs.device)
        # # position_encoding = self.position_embeddings(position_inputs).reshape(1, inputs.shape[1], inputs.shape[2]).repeat(inputs.shape[0], 1, 1)
        # position_encoding = torch.mm(position_inputs, self.position_embeddings).reshape(1, inputs.shape[1], inputs.shape[2]).repeat(inputs.shape[0], 1, 1)
        # temporal_inputs = inputs + position_encoding  # [N, T, F]
...
class ATT_layer_test(nn.Module):
    '''
    inputs may have different time-series lengths
    Method 1: Pad zero to make all inputs have the same lengths
    '''
    def __init__(self,
                ...
                residual,
                interval_ratio):
        super(ATT_layer_test, self).__init__()
        self.n_heads = n_heads
        self.residual = residual
        # self.sample_masks = sample_masks
        self.method = method
