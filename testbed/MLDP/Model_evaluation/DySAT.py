...
from layers_mb import GAT_Layer_test as StructuralAttentionLayer
from layers_mb import ATT_layer_test as TemporalAttentionLayer
...
def _structural_comm_nodes(adjs_list, local_workload_GCN):
    ...
    receive_list = []
    send_list = []
    # print('workload length', len(local_workload_GCN))
    # print('adj length', len(adjs_list))
...
def _structural_comm(args, features, workload_GCN, send_list, receive_list, node_size, bandwidth):
    ...
    features_dense = features.to('cpu')
    # print(features_dense)
    gather_lists = [torch.zeros_like(features_dense).to('cpu') for j in range(world_size)]
    comm_start = time.time()
    torch.distributed.all_gather(gather_lists, features_dense, group=dp_group)
    args['real_str_comm'] += time.time() - comm_start

    # feature fusion
    # features = features.to_sparse().to('cpu')
    features_dense = features_dense.to('cpu')
    gather_lists = [gather_lists[m].to('cpu') for m in range(world_size)]
    # final_feature = features_dense.clone().detach()
    for m in range(world_size):
        if receive_list != torch.Size([]) and receive_list.size(0) > 0:
            receive_list = torch.unique(receive_list)
            ...
            features_dense[have_node] = gather_lists[m][have_node]
    features_sp = features_dense
...
def _temporal_comm(args, embedding, workload_GCN, send_list, receive_list, node_size, bandwidth):
    ...
    # TODO: communicaiton component
    gather_lists = [torch.zeros_like(embedding).to('cpu') for j in range(world_size)]
    comm_start = time.time()
    torch.distributed.all_gather(gather_lists, embedding, group=dp_group)
    args['real_tem_comm'] += time.time() - comm_start
    ...
    # simulated communication time
    receive_node = receive_list.size(0)
    receive_time = np.around(float(receive_node*node_size*8)/bandwidth, 3)
    send_node = send_list.size(0)
    send_time = np.around(float(send_node*node_size*8)/bandwidth, 3)
    comm_time = max(receive_time, send_time)
...
class DySAT(nn.Module):
    ...
    def forward(self, graphs, gate, distribute):
        ...
        '''
        if distribute == True:
            GCN_emb_list = torch.ones(self.args['nodes_info'][-1], self.args['timesteps'], self.temporal_layer_config[-1])
            RNN_emb_tensor = torch.ones(self.args['nodes_info'][-1], self.args['timesteps'], self.temporal_layer_config[-1])
            ...
            num_GCN = 0
            # import wandb
            # wandb.init(project="DGNN_GPU_utilization", entity="fahao", name="GNN")
            ...
            for t in range(self.args['timesteps']):
                ...
                if self.args['dp']: # data parallel
                    _, fusion_features = _structural_comm(self.args, features, [self.workload_GCN[m][t] for m in range(self.args['world_size'])], send_list[t], receive_list[t], self.num_features*64, bandwidth=float(10*1024*1024*8))
                    graphs[t].ndata['feat'] = fusion_features
                if node_local_idx != torch.Size([]) and node_local_idx.size(0) > 0:
                    if self.args['data_str'] == 'dgl':
                        ...
                        subgraph = graphs[t].subgraph(node_idx.tolist())
                        # start = time.time()
                        out = self.structural_attn(subgraph).to('cpu')
                        # self.args['gcn_time'] += time.time() - start
                        # GCN_emb_list[t][node_idx] = out[:,None,:]  # to [N, 1, F]
                        # GCN_emb_list[t] = GCN_emb_list[t].to(self.device)
                        GCN_emb_list[node_idx, t, :][:, None, :] = out[:, None, :]  # to [N, 1, F]
                        structural_out.append(out)
                        num_GCN += node_local_idx.size(0)
                        graphs[t] = graphs[t].to('cpu')
                    else: return 0
            
            # print('number of GCN: ', num_GCN)
            ...
            for t in range(self.args['timesteps']):
                ...
                if self.args['dp']: # data parallel
                    _, fusion_embedding = _temporal_comm(self.args, GCN_emb_list[:,t,:], [self.workload_GCN[m][t] for m in range(self.args['world_size'])], send_list[t], receive_list[t], self.structural_layer_config[-1], bandwidth=float(10*1024*1024*8))
                    GCN_emb_list[:,t,:] = fusion_embedding

            temporal_output = []
            # temporal_input = torch.cat(GCN_emb_list, dim=1)
            # temporal_input = GCN_emb_list
            # # each node compute temporal out once
            # for t in range(self.args['timesteps']):
            ...
            # same nodes in different snapshots compute temporal out once
            num_RNN = 0
            already_compute_nodes_mask = torch.zeros(self.args['nodes_info'][-1], dtype=torch.bool)
            for t in range(self.args['timesteps'])[::-1]: # last snapshot as the first
                # send_list, receive_list = _temporal_comm_nodes(self.args['adjs_list'], self.local_workload_GCN)
                node_idx = torch.nonzero(self.local_workload_RNN[t] == True, as_tuple=False).view(-1)
                nodes_mask = already_compute_nodes_mask[node_idx]
                node_compute = torch.nonzero(nodes_mask == False, as_tuple=False).view(-1)
                # print('node idx:', node_idx.size(0))
                if node_compute != torch.Size([]) and node_compute.size(0) > 0:
                    # check already compute nodes
                    emb_input = GCN_emb_list[node_compute,:t+1,:]
                    emb_input = emb_input.to(self.device)
                    # print(emb_input.size())
                    # start = time.time()
                    out = self.temporal_attn(emb_input).to('cpu')
                    # self.args['att_time'] += time.time() - start
                    RNN_emb_tensor[node_compute, :t+1] = out
                    # RNN_emb_list[t][node_compute] = out[:,None,:]
                    # temporal_output.append(out)
                    # update already compte nodes
                    already_compute_nodes_mask[node_compute] = torch.ones(node_compute.size(0), dtype=torch.bool)
                    num_RNN += node_compute.size(0)
            # print('number of RNN: ', num_RNN)
            return RNN_emb_tensor
        else:
            # print("traing DGMM model locall!")
            # Structural Attention forward
            structural_out = []
...
class test_dysat(nn.Module):
    def __init__(self, args, num_features):
        ...
        Args:
            ...
            method: adding time interval information methods
        '''
        super(test_dysat, self).__init__()
        # structural_time_steps = args['structural_time_steps']
        # temporal_time_steps = args['temporal_time_steps']
        self.args = args
        # for local training
    ...
    def forward(self, graphs):
        ...
        for t in range(0, len(graphs)):
            full_nodes.extend(graphs[t].nodes().tolist())
            nodes_list_per_snapshot.append(graphs[t].nodes().tolist())
            # graphs[t] = graphs[t].to(self.device)
            start = time.time()
            structural_out.append(self.structural_attn(graphs[t]))
            total_cost += time.time() - start
            time_per_operation.append(time.time() - start)
        # structural_outputs = [g[:,None,:] for g in structural_out] # list of [Ni, 1, F]
        ...
        timesteps = len(graphs)
        tempora_feat_tensor = torch.zeros(len(full_nodes), timesteps, 128, dtype=torch.float32)  # [N,T,F]
        ...
        for t in range(timesteps):
            ...
            tempora_feat_tensor[idx, t, :] = structural_out[t]

        # Temporal Attention forward
        start = time.time()
        temporal_out = self.temporal_attn(tempora_feat_tensor)
        total_cost += time.time() - start
        
        # return temporal_out
        # return time.time() - start
        time_per_operation.append(time.time() - start)
        # return total_cost, time_per_operation
        return time.time() - start_all, time_per_operation
